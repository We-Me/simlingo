...
# Generate four-view DriveLM/VQA labels from a collected CP dataset.
#
# Common overrides:
#   DATASET_ROOT=/path/to/dataset
#   VQA_OUTPUT_DIRECTORY=/path/to/output
#   WORKERS=1
...
OUTPUT_DIRECTORY="${VQA_OUTPUT_DIRECTORY:-$DATASET_ROOT/drivelm}"
WORKERS="${WORKERS:-1}"
...
echo "Generating DriveLM/VQA labels"
echo "  dataset: $DATASET_ROOT"
echo "  output:  $OUTPUT_DIRECTORY"

exec "$PYTHON_BIN" \
    dataset_generation/language_labels/drivelm/carla_vqa_generator_main_cp.py \
    "${ARGS[@]}" "$@"
